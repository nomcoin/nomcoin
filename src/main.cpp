...
const string strMessageMagic = "nomcoin Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // nomcoin: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal nomcoin miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in nomcoin
    bool fStrictPayToScriptHash = true; // Always active in nomcoin
    ...
    // ppcoin: fees are not collected by miners as in nomcoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "nomcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "nomcoin - .";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:nomcoin genesis block";
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("nomcoinMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("nomcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("nomcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadnomcoinMiner(void* parg);

static bool fGeneratenomcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void nomcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("nomcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGeneratenomcoins || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGeneratenomcoins) && !fProofOfStake)
                return;
        }
        ...
        printf("Running nomcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadnomcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        nomcoinMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadnomcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadnomcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadnomcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void Generatenomcoins(bool fGenerate, CWallet* pwallet)
{
    fGeneratenomcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGeneratenomcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d nomcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadnomcoinMiner, pwallet))
                printf("Error: NewThread(ThreadnomcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
